...
public class LoginQueryHandler implements QueryHandler<TokenModel, LoginQuery> {
    ...
    public TokenModel handle(LoginQuery query) throws Exception {
        //Con finjo que los datos salieron de un formuñario html
        ...
        //Con WebFlux hago un request a /login, este redirige a AuthenticationFilterForLogin
        var headers =
                WebClient.builder().baseUrl("http://localhost:8080").build()
                .post().uri("/login")
                .contentType(MediaType.APPLICATION_JSON).accept(MediaType.ALL)
                .body(BodyInserters.fromFormData(keys))
